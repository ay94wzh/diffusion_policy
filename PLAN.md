...
- **M4 — Per-view aux heads.** ✅ **DONE (2026-09-22) — RESULT: a mechanistically-live, behaviourally-null A/B.** `m4on` − `m4off` = +0.02 trained / +0.04 held-out on square, inside the ±0.05 noise, and M4 sits inside M3's band. The pair is the project's only **RNG-locked** A/B (batch-0 identity exact to `0.000e+00`) and the ablation is confirmed at weight level (`m4off`'s aux head drifts 1e-5). But the mechanism is *real*: `aux_loss` falls **52×** (0.3415 → 0.0066) and reaches **6.4%** of a mean-collapsing predictor's floor, so the head provably reads view-relative geometry from `z_v` — M3's failure mode, fixed in the measurable sense. A weight-space diagnostic shows the aux term moves the EMA weights **about as much as re-seeding does** (UNet 0.856 vs an RNG-only reference of 0.824), which kills "the gradient was too weak" as the explanation; the surviving one is that **the information was already in `z_v`**. New files: `policy/diffusion_unet_image_policy_aux.py`, `model/vision/per_view_aux_head.py`, `config/task/m4_aux_image_abs_multiview.yaml`, `config/train_diffusion_unet_image_workspace_m4.yaml`, `tests/test_aux_action_heads.py`; `multiview_image_dataset.py` gained `action_to_cam()` and emits the chunk as a **top-level** key. Full tables, the auxiliary diagnostics, and the elevation caveat (`el_0` vs `az_0` differ by **0.14 on the same camera pose**, so the effective noise exceeds ±0.05): `PROGRESS.md` §13.
- **§6's architectural confound — resolved as its own milestone.** ✅ **DONE (2026-09-23).** `run_square_m3fixedn1_s42_200ep`: M3-off's exact model with one CLI line changed (`task.dataset.view_count_range=[1,1]`), so every sample has one active slot and the view is drawn from the same 7-pose pool. **It tracks L1, not m3off** — 0.04 trained / 0.05 held-out vs `m3off`'s 0.76 / 0.55, and 0.00–0.06 at all eleven azimuths, with `el_0 = 0.02` on the trained pose. So of the four changes that arrived as plumbing, **multi-view sampling (N>1) is the load-bearing one**; the encoder architecture alone reproduces nothing. This closes the last candidate — conditioning (§11.3) and aux pressure (§13) were already inert. Two things it does *not* settle: N=1 *inference* (M3 infers at N=1 fine, because it trained with N>1), and "N>1 needed" vs "variable N needed" (`[2,7]` would separate them; `[2,2]` answers PROPOSAL §7's how-much-diversity). Details: `PROGRESS.md` §14.
- **M5 — Single-novel-view inference (+ optional distillation).** Fusion already supports N=1 **and M3 is trained with randomized N precisely so this works**; at eval the novel camera pose is known from the sim, and `eval_novel_view.py --m3-slots K` already publishes the perturbed pose into the slot's cam key. Optional teacher→student distillation (single-view encoder regresses the multi-view fused latent). *Gate:* final sweep tables vs the Milestone 1 baseline curves. **Premise note after §14:** M3's N=1 inference works *because* training included N>1, so the capability lives in the training signal — distillation is only worth it if the fused latent is shown to carry something the single-view path cannot, which no result so far demonstrates.
