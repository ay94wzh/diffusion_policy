...
- **L1 — view-randomized single-slot baseline ("view diversity only").** ✅ **DONE for square/seed 42 (2026-09-16); replication running.** M1's *exact* architecture, with its one camera slot filled per sample from a randomly drawn training view (the 7 even ring indices, every 30°; the odd azimuths ±15/±45/±75 are never trained on). It scores **0.02 at az_0 — a pose it trained on — where M1 scores 0.82**, and is flat at ~0 across all 11 evaluated viewpoints, trained and held out alike. So naive view diversity without pose information does not buy view invariance; it destroys the policy at every pose while still fitting the demonstrations. This is the baseline M3 must beat, and the empirical motivation for pose conditioning. Method, tables and stated limits in `PROGRESS.md` §8.
